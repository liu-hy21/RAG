...
# 包含 document_id 的最小化字段定义
fields = [
    FieldSchema(name="pk", dtype=DataType.INT64, is_primary=True, auto_id=True),
    FieldSchema(name="document_id", dtype=DataType.INT64),
    FieldSchema(name="chunk_hash", dtype=DataType.VARCHAR, max_length=64),
    FieldSchema(name="chunk_text", dtype=DataType.VARCHAR, max_length=60000),
    FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, dim=1024)  # bge-large-en-v1.5 输出维度为 1024
]

schema = CollectionSchema(
    fields=fields,
    description="Minimized document chunks collection with document_id",
    enable_dynamic_field=False  # 最小化时可关闭动态字段
)
collection_name = "minimized_documents_collection_with_doc_id"
...
def insert_optimized_data():
    ...
    # 加载 bge-large-en-v1.5 模型
    model = SentenceTransformer('BAAI/bge-large-en-v1.5', device='cpu')

    # 批量处理参数
    file_path = "/Users/shione/python/nlp/project/jsonl/documents_markdown_clean.jsonl"
    batch_size = 200
    batch_data = []
